...
const Portfolio = () => {
  ...
  return (
    <div className="bg-[#0b101d] text-slate-300 min-h-screen font-sans antialiased selection:bg-cyan-500 selection:text-black overflow-x-hidden">
      ...
      {/* Hero Section */}
      <header className="max-w-6xl mx-auto px-4 sm:px-6 py-12 md:py-20 flex flex-col-reverse md:flex-row items-center justify-between gap-10">
        <div className="max-w-2xl text-left">
          <p className="text-xs font-mono tracking-widest text-cyan-400 uppercase mb-4">Hi there, I am</p>
          <h1 className="text-4xl sm:text-6xl font-extrabold text-white tracking-tight mb-4">
            {profileData.name}
          </h1>
          <h2 className="text-xl sm:text-3xl font-semibold text-slate-400 mb-6">
            {profileData.tagline}
          </h2>
          <p className="text-slate-400 leading-relaxed text-sm sm:text-base mb-8 max-w-xl">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <a href="#projects" className="px-5 py-3 text-sm font-semibold text-black bg-cyan-400 rounded-lg shadow-lg shadow-cyan-500/20 hover:bg-cyan-300 transition">
              View My Work
            </a>
            <a href="#contact" className="px-5 py-3 text-sm font-semibold text-slate-200 border border-slate-700 rounded-lg hover:border-slate-500 hover:text-white transition">
              Get In Touch
            </a>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-20 md:space-y-28 pb-20">

        {/* 01. About Me */}
        <section id="about" className="scroll-mt-10">
          <div className="flex items-center gap-4 mb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
              <span className="text-cyan-400 font-mono text-lg sm:text-xl">01.</span> About Me
            </h2>
        ...
        <section id="skills" className="scroll-mt-10">
          <div className="flex items-center gap-4 mb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
              <span className="text-cyan-400 font-mono text-lg sm:text-xl">02.</span> Core Capabilities
            </h2>
            <div className="h-[1px] bg-slate-800 flex-1 max-w-md"></div>
          </div>
        ...
        <section id="projects" className="scroll-mt-10">
          <div className="flex items-center gap-4 mb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
              <span className="text-cyan-400 font-mono text-lg sm:text-xl">03.</span> Selected Projects
            </h2>
            <div className="h-[1px] bg-slate-800 flex-1 max-w-md"></div>
          </div>
            ...
            {projects.map((proj, idx) => (
              <div key={idx} className="bg-[#111726]/90 border border-slate-800 p-6 rounded-2xl flex flex-col justify-between hover:border-cyan-500/40 transition group">
                <div>
                  ...
                  <h3 className="text-lg sm:text-xl font-bold text-white mb-2 group-hover:text-cyan-400 transition">{proj.title}</h3>
        ...
        <section id="contact" className="scroll-mt-10 text-center max-w-2xl mx-auto pt-6">
          ...
          <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-8">
            <a href={`mailto:${profileData.email}`} className="bg-[#111726] border border-slate-800 hover:border-cyan-500/50 px-4 py-3 rounded-xl flex items-center gap-2 text-xs sm:text-sm text-slate-200 transition">
              ✉️ {profileData.email}
            </a>
            <a href={`https://wa.me/${profileData.phone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noreferrer" className="bg-[#111726] border border-slate-800 hover:border-cyan-500/50 px-4 py-3 rounded-xl flex items-center gap-2 text-xs sm:text-sm text-emerald-400 transition">
              💬 WhatsApp
            </a>
            <a href={profileData.linkedin} target="_blank" rel="noreferrer" className="bg-[#111726] border border-slate-800 hover:border-cyan-500/50 px-4 py-3 rounded-xl flex items-center gap-2 text-xs sm:text-sm text-blue-400 transition">
              🔗 LinkedIn
            </a>
            <a href={profileData.github} target="_blank" rel="noreferrer" className="bg-[#111726] border border-slate-800 hover:border-cyan-500/50 px-4 py-3 rounded-xl flex items-center gap-2 text-xs sm:text-sm text-slate-300 transition">
              💻 GitHub
            </a>
          </div>
        </section>
